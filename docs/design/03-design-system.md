...
Implemented as CSS custom properties and component classes in `new/css/site.css`. `compliance.css` (accessibility widget, cookie consent) consumes the same tokens. There is no build step: tokens are plain CSS variables on `:root`.
